...
== Common application properties 常见的应用属性集
...
本节提供了常见的Spring Boot属性集和对使用它们的基础类的引用。
...
----
	...
	# BANNER
	# 系统品牌的横幅广告(banner.)
	banner.charset=UTF-8 # Banner file encoding. 横幅文件的编码
	banner.location=classpath:banner.txt # Banner file location. 横幅文件的位置
	...
	# LOGGING
	# 日志(logging.)
	logging.config= # Location of the logging configuration file. For instance `classpath:logback.xml` for Logback 日志记录配置文件的位置
	logging.exception-conversion-word=%wEx # Conversion word used when logging exceptions.
	logging.file= # Log file name. For instance `myapp.log` 日志文件名
	logging.level.*= # Log levels severity mapping. For instance `logging.level.org.springframework=DEBUG` 日志级别严重性映射
	logging.path= # Location of the log file. For instance `/var/log` 日志文件的位置
	...
	# 应用上下文ID(spring.application.)
	spring.application.index= # Application index. 应用程序的索引
	spring.application.name= # Application name. 应用程序的名称
	...
	# 基于JMX监控的管理员(spring.application.admin.)
	spring.application.admin.enabled=false # Enable admin features for the application. 为应用程序启用管理员功能
	spring.application.admin.jmx-name=org.springframework.boot:type=Admin,name=SpringApplication # JMX name of the application admin MBean. 应用程序管理员MBean的JMX名称

	# AUTO-CONFIGURATION
	# 自动配置(spring.autoconfigure.)
	spring.autoconfigure.exclude= # Auto-configuration classes to exclude. 排除的自动配置类列表
	...
	spring.cache.type= # Cache type, auto-detected according to the environment by default. 缓存类型，默认情况下根据环境自动检测
	...
	spring.config.location= # Config file locations. 配置文件位置列表
	spring.config.name=application # Config file name. 配置文件名
	...
	# JMX
	# 基于JMX实现的监控体系(spring.jmx.)
	spring.jmx.default-domain= # JMX domain name. JMX的域名
	spring.jmx.enabled=true # Expose management beans to the JMX domain. 暴露管理beans到JMX域
	spring.jmx.server=mbeanServer # MBeanServer bean name. 管理bean服务器的bean名称
	...
	# Spring应用程序设置(spring.main.)
	spring.main.banner-mode=console # Mode used to display the banner when the application runs. 应用程序运行时用于显示横幅的模式
	spring.main.sources= # Sources (class name, package name or XML resource location) to include in the ApplicationContext. 包含在应用上下文中的bean资源对象集(类名，包名或XML资源位置)
	...
	# 文件编码
	spring.mandatory-file-encoding= # Expected character encoding the application must use. 应用程序必须使用的预期的字符编码
	...
	# Spring应用程序进程ID(spring.pid.)
	spring.pid.fail-on-write-error= # Fail if ApplicationPidFileWriter is used but it cannot write the PID file. 如果使用ApplicationPidFileWriter但无法写入PID文件，则失败
	spring.pid.file= # Location of the PID file to write (if ApplicationPidFileWriter is used). 要写入的PID文件的位置(如果使用了ApplicationPidFileWriter)

	# PROFILES
	# 配置文件集(spring.profiles.)
	spring.profiles.active= # Comma-separated list (or list if using YAML) of <<howto-set-active-spring-profiles,active profiles>>. 活动的配置文件集的逗号分隔列表
	spring.profiles.include= # Unconditionally activate the specified comma separated profiles (or list of profiles if using YAML). 无条件激活指定的逗号分隔的配置文件集
	...
	server.compression.enabled=false # If response compression is enabled. 启用响应压缩
	...
	server.compression.min-response-size=2048 # Minimum response size that is required for compression to be performed. 执行压缩所需的最小响应大小(2KB)
	server.connection-timeout= # Time in milliseconds that connectors will wait for another HTTP request before closing the connection. When not set, the connector's container-specific default will be used. Use a value of -1 to indicate no (i.e. infinite) timeout. 连接器在关闭连接之前将等待另一个HTTP请求的时间(毫秒)。未设置时，将使用连接器特定于容器的默认值。
	server.context-parameters.*= # Servlet context init parameters. For instance `server.context-parameters.a=alpha`
	server.context-path= # Context path of the application. 应用程序的上下文路径
	server.display-name=application # Display name of the application. 应用程序的显示名称
	server.max-http-header-size=0 # Maximum size in bytes of the HTTP message header.
	server.error.include-stacktrace=never # When to include a "stacktrace" attribute.
	server.error.path=/error # Path of the error controller. 错误控制器的路径
	...
	server.port=8080 # Server HTTP port. 服务器HTTP请求端口
	server.server-header= # Value to use for the Server response header (no header is sent if empty)
	server.servlet-path=/ # Path of the main dispatcher servlet. 主调度程序的路径
	server.use-forward-headers= # If X-Forwarded-* headers should be applied to the HttpRequest.
	server.session.cookie.comment= # Comment for the session cookie.
	server.session.cookie.domain= # Domain for the session cookie.
	server.session.cookie.http-only= # "HttpOnly" flag for the session cookie.
	server.session.cookie.max-age= # Maximum age of the session cookie in seconds. 会话cookie的最长期限(秒)
	server.session.cookie.name= # Session cookie name.
	server.session.cookie.path= # Path of the session cookie.
	server.session.cookie.secure= # "Secure" flag for the session cookie.
	server.session.persistent=false # Persist session data between restarts.
	server.session.store-dir= # Directory used to store session data.
	server.session.timeout= # Session timeout in seconds. 会话超时(秒)
	server.session.tracking-modes= # Session tracking modes (one or more of the following: "cookie", "url", "ssl").
	server.ssl.ciphers= # Supported SSL ciphers.
	server.ssl.client-auth= # Whether client authentication is wanted ("want") or needed ("need"). Requires a trust store.
	server.ssl.enabled= # Enable SSL support. 启用SSL支持
	server.ssl.enabled-protocols= # Enabled SSL protocols. 启用SSL协议
	...
	server.ssl.protocol=TLS # SSL protocol to use. 要使用的SSL协议
	server.ssl.trust-store= # Trust store that holds SSL certificates.
	server.ssl.trust-store-password= # Password used to access the trust store.
	server.ssl.trust-store-provider= # Provider for the trust store.
	server.ssl.trust-store-type= # Type of the trust store.
	# Tomcat配置(server.tomcat.)
	server.tomcat.accept-count=100 # Maximum queue length for incoming connection requests when all possible request processing threads are in use. 当所有可能的请求处理线程都在使用时，进来的连接请求的最大队列长度
	server.tomcat.accesslog.buffered=true # Buffer output such that it is only flushed periodically. 缓冲输出，使其仅定期刷新
	server.tomcat.accesslog.directory=logs # Directory in which log files are created. Can be relative to the tomcat base dir or absolute. 在其中创建日志文件的目录，可以是tomcat基本目录的相对目录
	server.tomcat.accesslog.enabled=false # Enable access log. 启用访问日志
	server.tomcat.accesslog.file-date-format=.yyyy-MM-dd # Date format to place in log file name. 要放在日志文件名中的日期格式
	server.tomcat.accesslog.pattern=common # Format pattern for access logs. 访问日志的格式模式
	server.tomcat.accesslog.prefix=access_log # Log file name prefix. 日志文件名前缀
	server.tomcat.accesslog.rename-on-rotate=false # Defer inclusion of the date stamp in the file name until rotate time. 将日期时间戳包含在文件名中，直到轮换时间为止
	server.tomcat.accesslog.request-attributes-enabled=false # Set request attributes for IP address, Hostname, protocol and port used for the request. 设置用于请求的IP地址，主机名，协议和端口的请求属性
	server.tomcat.accesslog.rotate=true # Enable access log rotation. 启用访问日志循环输出
	server.tomcat.accesslog.suffix=.log # Log file name suffix. 日志文件名后缀
	...
	server.tomcat.max-connections=10000 # Maximum number of connections that the server will accept and process at any given time. 服务器在任何给定时间接受和处理的最大连接数
	server.tomcat.max-http-header-size=0 # Maximum size in bytes of the HTTP message header.
	server.tomcat.max-http-post-size=2097152 # Maximum size in bytes of the HTTP post content. HTTP发布内容的最大字节大小(2MB)
	server.tomcat.max-threads=200 # Maximum amount of worker threads. 最大工作者线程数
	server.tomcat.min-spare-threads=10 # Minimum amount of worker threads. 最小工作者线程数
	server.tomcat.port-header=X-Forwarded-Port # Name of the HTTP header used to override the original port value.
	server.tomcat.protocol-header= # Header that holds the incoming protocol, usually named "X-Forwarded-Proto".
	server.tomcat.protocol-header-https-value=https # Value of the protocol header that indicates that the incoming request uses SSL.
	server.tomcat.redirect-context-root=true # Whether requests to the context root should be redirected by appending a / to the path.
	server.tomcat.remote-ip-header= # Name of the http header from which the remote ip is extracted. For instance `X-FORWARDED-FOR`
	server.tomcat.uri-encoding=UTF-8 # Character encoding to use to decode the URI. 用于解码URI的字符编码
	...
	# Groovy模板(spring.groovy.template.)
	...
	# HTTP消息转换(spring.http.converters.)
	spring.http.converters.preferred-json-mapper=jackson # Preferred JSON mapper to use for HTTP message conversion. Set to "gson" to force the use of Gson when both it and Jackson are on the classpath. 用于HTTP消息转换的首选JSON映射器

	# HTTP encoding ({sc-spring-boot-autoconfigure}/web/HttpEncodingProperties.{sc-ext}[HttpEncodingProperties])
	# HTTP协议编码(spring.http.encoding.)
	spring.http.encoding.charset=UTF-8 # Charset of HTTP requests and responses. Added to the "Content-Type" header if not set explicitly. HTTP请求和响应的字符集。如果未明确设置，则添加到"Content-Type"头信息中
	spring.http.encoding.enabled=true # Enable http encoding support. 启用HTTP编码支持
	...
	# 文件上传(spring.http.multipart.)
	spring.http.multipart.enabled=true # Enable support of multi-part uploads. 启用对文件多部分上传的支持
	...
	spring.http.multipart.max-file-size=1MB # Max file size. Values can use the suffixed "MB" or "KB" to indicate a Megabyte or Kilobyte size. 最大文件大小
	spring.http.multipart.max-request-size=10MB # Max request size. Values can use the suffixed "MB" or "KB" to indicate a Megabyte or Kilobyte size. 最大请求大小
	spring.http.multipart.resolve-lazily=false # Whether to resolve the multipart request lazily at the time of file or parameter access. 在文件或参数访问时是否延迟解析多部分请求

	# JACKSON ({sc-spring-boot-autoconfigure}/jackson/JacksonProperties.{sc-ext}[JacksonProperties])
	# Jackson配置(spring.jackson.)
	spring.jackson.date-format= # Date format string or a fully-qualified date format class name. For instance `yyyy-MM-dd HH:mm:ss`. 日期格式字符串或完全限定日期格式类名称
	spring.jackson.default-property-inclusion= # Controls the inclusion of properties during serialization. 控制序列化期间属性的包含
	...
	spring.jackson.property-naming-strategy= # One of the constants on Jackson's PropertyNamingStrategy. Can also be a fully-qualified class name of a PropertyNamingStrategy subclass. Jackson的属性名称策略接口的常量之一，也可以是其子类的完全限定类名称
	...
	# Spring移动设备视图(spring.mobile.devicedelegatingviewresolver.)
	...
	# Spring模型-视图-控制器(spring.mvc.)
	spring.mvc.async.request-timeout= # Amount of time (in milliseconds) before asynchronous request handling times out. 异步请求处理超时之前的时间(毫秒)
	spring.mvc.date-format= # Date format to use. For instance `dd/MM/yyyy`. 要使用的日期格式
	...
	spring.mvc.servlet.load-on-startup=-1 # Load on startup priority of the Spring Web Services servlet. 加载servlet的启动优先级
	...
	security.basic.enabled=true # Enable basic authentication. 启用基本的身份验证
	security.basic.path=/** # Comma-separated list of paths to secure. 以逗号分隔的安全路径列表
	security.basic.realm=Spring # HTTP basic realm name.
	security.enable-csrf=false # Enable Cross Site Request Forgery support. 启用跨站点请求伪造支持
	security.filter-order=0 # Security filter chain order. 安全过滤器链的顺序
	security.filter-dispatcher-types=ASYNC, FORWARD, INCLUDE, REQUEST # Security filter chain dispatcher types. 安全筛选器链的调度程序类型
	...
	# 端点列表
	endpoints.enabled=true # Enable endpoints. 启用端点
	endpoints.sensitive= # Default endpoint sensitive setting.
	endpoints.actuator.enabled=true # Enable the endpoint.
	endpoints.actuator.path= # Endpoint URL path. 端点URL路径
	...
	endpoints.jmx.enabled=true # Enable JMX export of all endpoints. 启用所有端点的JMX导出
	...
	management.port= # Management endpoint HTTP port. Uses the same port as the application by default. Configure a different port to use management-specific SSL. 管理端点的HTTP端口。默认情况下，使用与应用程序相同的端口。
	management.security.enabled=true # Enable security. 启用安全性
	...
	management.health.defaults.enabled=true # Enable default health indicators. 启用默认的运行状况指示器
	...
	management.trace.include=request-headers,response-headers,cookies,errors # Items to be included in the trace. 要包含在追踪中的项目
